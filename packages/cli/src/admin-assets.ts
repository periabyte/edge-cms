...
/**
 * Locates the built admin SPA (`@edgecms/admin`'s `dist/`) so `dev` and
 ...
 *
 * Resolution is relative to this CLI module, which declares `@edgecms/admin`
...
export function resolveAdminDist(): string | undefined {
  try {
    ...
    const pkgJson = require.resolve("@edgecms/admin/package.json");
